...
function createManualModeService({ logger, waClient, markMessageAsSentByBot, isMessageSentByBot }) {
    ...
    function processOutgoingMessage(message, clinicaId, number) {
        ...
        // Ignorar mensagem de reset de saudação
        ...
        if (isBotMessage) {
            ...
            logger.log(`Outgoing message identified as bot, chatbot remains active for ${number}`);
        } else {
            // Exibir mensagem para ajudar na depuração
            const messageKey = `bot_msg:${clinicaId}:${messageBody.substring(0, 50)}`;
            logger.log(`Outgoing message appears manual (key not found: ${messageKey}), message preview: "${messageBody.substring(0, 50)}"`);
