<template>
  <div class="bg-white rounded-lg shadow p-4">
    <h2 class="font-medium text-lg mb-3">Resumo do Paciente</h2>
    
    <div class="space-y-4">
      <!-- Dados básicos do paciente -->
      <div class="flex items-start">
        <div class="h-14 w-14 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden mr-4">
      ...
      <div class="grid grid-cols-4 gap-3 mt-4">
        <div v-for="(vital, index) in patient.vitals" :key="index" class="bg-gray-50 p-3 rounded-lg">
      ...
      <div class="space-y-2 mt-4">
        <div v-for="(alert, index) in patient.alerts" :key="index" 
          :class="`p-2 rounded-lg text-sm flex items-center ${
            ...
            alert.type === 'warning' ? 'bg-orange-50 text-orange-700 border border-orange-100' : 
      ...
      <div class="mt-4">
        <h4 class="font-medium text-sm text-gray-700 mb-2">Medicações de uso contínuo</h4>
        <div class="space-y-2">
          <div v-for="(med, index) in patient.continuousMeds" :key="index" class="bg-gray-50 p-2 rounded-lg text-sm">
            <p class="font-medium">{{ med.name }}</p>
            <p class="text-gray-500 text-xs">{{ med.dosage }} - {{ med.frequency }}</p>
          </div>
        </div>
      </div>
    </div>
...
import { User, Calendar, AlertCircle } from 'lucide-vue-next';
